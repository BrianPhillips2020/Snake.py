...
class App:
   ...
   #def on_event(self, event):
    ...
    def on_loop(self):
        ...
        #player border collissionend == False 
    ...
    def on_execute(self):  
        ...
        while(self._running):
            ...
            if self.player.direction == 'left':
                ...
                elif self.player.body[0][0] == 0:
                    self.player.moveUp()
            elif self.player.direction == 'right':
                #print("MOVE")
                if self.player.body[0][1] == 0:
                    self.player.moveDown()
                elif self.player.body[0][1] == 560: 
                    self.player.moveUp()
            elif self.player.body[0][0] == 780 and self.player.body[0][1] == 560:
                self.player.moveDown()
            elif self.player.body[0][0] ==780 and self.player.body[0][1] == 580:
                self.player.moveLeft()
            elif self.player.direction == 'up' and self.player.body[0][1] == 0:
                #print("yeehaw")
                self.go_right = True
                self.player.moveRight()
                self.update_player_pos()
                #self.player.body[0][0] = self.player.body[0][0] + self.player.speed
            elif self.player.direction == 'down' and self.player.body[0][1] == 560:
                self.go_right = True
                self.player.moveRight()
                self.update_player_pos()
                #self.player.body[0][0] = self.player.body[0][0] + self.player.speed
